...
const (
	...
	ERR_TPL_BINDING_RESERVED_WORD_IN_STRICT_MODE   = "Binding `%s` in strict mode"
	...
	ERR_FN_DEC_SHOULD_TOP_OR_IN_BLOCK              = "In strict mode code, functions can only be declared at top level or inside a block"
	ERR_STATIC_PROP_PROTOTYPE                      = "Classes may not have a static property named `prototype`"
	ERR_YIELD_CANNOT_BE_DEFAULT_VALUE              = "Yield expression cannot be a default value"
	ERR_YIELD_IN_FORMAL_PARAMS                     = "Yield expression is not allowed in formal parameters"
